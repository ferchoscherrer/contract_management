...
private _askPostventa(sPathMaterial: string, oMaterialSelected: any): void {
    ...
    private _headerIn(): QuotationHeaderIn {
        ...
        return {
            ...
            ReqDateH: this._formatDateForBackend(oHeader.orderDate),
        CtValidF: this._formatDateForBackend(oHeader.validFromDate),
        CtValidT: this._formatDateForBackend(oHeader.validUntilDate),
        QtValidF: this._formatDateForBackend(oHeader.validFromDateQuo),
        QtValidT: this._formatDateForBackend(oHeader.validUntilDateQuo),
    ...
    private _itemsAndData(): ItemsAndData {
        ...
        const arrQuotationDataInSet: QuotationDataInSet[] = [{
            ...
            ConStDat: this._formatDateForBackend(oHeader.validFromDate),
            ConSiDat: new Date("0000000000000"),
            ConEnDat: this._formatDateForBackend(oHeader.validUntilDate),
            ActDatrul: "",
            CancDoc: "",
            CancParty: "",
            CancProc: "",
        ...
        for (let i = 0; i < arrMaterial.length; i++) {
            ...
            const oQuotationDataInSet: QuotationDataInSet = {
                ...
                AcceptDat: new Date("0000000000000"),
                ConStDat: this._formatDateForBackend(oMaterial.validFromDate),
                ConSiDat: new Date("0000000000000"),
                ConEnDat: this._formatDateForBackend(oMaterial.validUntilDate),
                ActDatrul: "",
                CancDoc: "",
                CancParty: "",
                CancProc: "",
    ...
    private _formatDateForBackend(oDate: any): string | any {
    // Si no es una fecha válida, regresamos el valor original o un string vacío
    if (!oDate || !(oDate instanceof Date) || isNaN(oDate.getTime())) {
        return oDate || "";
    }
    
    // Extraemos las partes locales de la fecha exactamente como el usuario las ve en la pantalla
    const yyyy = oDate.getFullYear();
    const MM = String(oDate.getMonth() + 1).padStart(2, "0");
    const dd = String(oDate.getDate()).padStart(2, "0");

    // Retornamos el formato neutral ISO sin offset de huso horario
